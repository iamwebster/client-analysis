...
class ClientAnalysis:
    ...
    def get_client_actions(self, dataframe: DataFrame | None = None) -> DataFrame:
        '''Подсчет числа действий каждого клиента.'''
        if dataframe:
            self.df = dataframe
        return self.df.groupby(['client_id', 'action']).count()
    
    def filter_client_data(self, client_id: int | None = None, 
                           action: str | None = None, timestamp: str | None = None, 
                           dataframe: DataFrame | None = None) -> DataFrame:
        '''
    ...
    def analyze_client_behavior(self, dataframe: DataFrame | None = None) -> DataFrame:
        '''Получение топ-5 клиентов с наибольшим числом действий.'''
        if dataframe:
            self.df = dataframe
        return self.df[['client_id', 'action']].groupby('client_id').\
            count().sort_values(['action'], ascending=False)[:5].reset_index()
    
    def save_processed_data(self, file_name: str, dataframe: DataFrame | None = None) -> bool:
        '''
